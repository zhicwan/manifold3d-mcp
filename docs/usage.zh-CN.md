...
向助手描述模型，校验生成的 TypeScript，检查结果，再导出 3MF 或 GLB。两种插件共享
建模引擎，但预览与反馈方式不同。
...
推荐流程是 **描述 → 按需澄清 → 建模 → 校验 → 检查 → 试制 → 修改**：

1. 提供用途和已知尺寸、配合要求。明确的简单请求可以直接推进；
   遇到影响方案的未知条件，助手一次澄清一个关键决策，并区分已确认事实与假设。
2. 当工艺影响设计时，先选定打印方式：FDM/FFF 是主要指引路径，
   树脂和聚合物 SLS 指引按需加载。详细建模前考虑接口、朝向、支撑和清理。
   条件不完整时，可以先生成明确标注假设的概念模型。
3. 让助手校验脚本，对照要求检查关键特征，而不只是整体包围盒。
   仅校验不会更新预览。
4. 准备展示某个版本时，执行已通过校验的脚本。Canvas 需要打开面板；
   MCP 如果没有自动打开浏览器，可打开返回的 `previewUrl`。
5. 从合适的角度检查模型，包括渲染截图。提出具体修改，例如
   “把走线槽加宽到 22 mm，其他部分保持不变。”
6. 每次几何修改后重复校验，并复查受影响的配合与工艺条件。
   导出后检查切片，必要时进行代表性试制；带着打印机、材料、设置和测量结果反馈，
   再调整相应参数。

临时候选、数值比较和子组件诊断可以只做校验，保持当前预览不变。
准备展示的版本再执行并截图，不必为每个中间变体更新预览。
...
英文[设计工作流](../skills/shared/references/design-workflow.md)说明助手如何兼顾
需求澄清、结构、外观与约束，而不把每个请求变成固定问卷。
...
在 Viewer 中选择 **Export → 3MF（用于打印）**。将 3MF 导入切片软件后，以毫米核对
尺寸，并检查打印朝向、支撑、壁厚、间隙与材料。多零件设计应先检查并分别排布
零件，再打印。

需要交给其他 3D 查看器或工具时，选择 **Export → GLB（用于查看与交换）**。
GLB 使用 glTF 的坐标与米制约定，同时保持模型的实际物理尺寸。

导出的 3MF 是带明确毫米单位的通用核心模型，不是 Bambu Studio 项目，也不包含
打印板、打印机或耗材配置、切片参数、缩略图或刀路。如需以后修改参数，请保留
TypeScript 源码；3MF 与 GLB 都是网格结果，不是参数化项目文件。

交付说明应区分概念模型、几何已检查但仍待切片/试制的模型，以及已有实测反馈的版本，
并列出关键尺寸与间隙、建议打印方向、材料假设和未解决的检查项。
改变观察视角并不改变导出的几何朝向。

配合不确定时，先打印有代表性的局部试片。反馈名义尺寸、实测尺寸、配合表现、
模型版本和加工处理条件；调整模型前先确认切片器中已有的补偿，
不要为修正一个接口而整体缩放零件。详见英文
[验收与交付指引](../skills/shared/references/verification-and-handoff.md)。
