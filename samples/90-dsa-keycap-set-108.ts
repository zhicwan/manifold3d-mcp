...
const CROSS_L = 4.10;       // MX cross arm length
...
const makeKeycap = (uX: number, uY: number = 1): Manifold => {
  ...
  const topSlab = Manifold.extrude(roundedRect(topX, topY, TOP_R), SHELL_SLAB)
    .translate([0, 0, HEIGHT - SHELL_SLAB]);
  ...
  if (longRatio < SPHERE_RATIO_LIMIT) {
    ...
    shell = shell.subtract(
      Manifold.sphere(dishR, segs).translate([0, 0, HEIGHT + dishR - h])
    );
  } else if (topX >= topY) {
  ...
  const innerBaseSlab = Manifold.extrude(
    roundedRect(baseX - 2 * WALL, baseY - 2 * WALL, BASE_R), SHELL_SLAB,
  ).translate([0, 0, -CAVITY_OVERSHOOT]);
  const innerTopSlab = Manifold.extrude(
    roundedRect(topX - 2 * WALL, topY - 2 * WALL, TOP_R), SHELL_SLAB,
  ).translate([0, 0, innerHeight - SHELL_SLAB]);
  ...
  const crossA = Manifold.cube([CROSS_L, CROSS_W, STEM_DEPTH + 0.5], true)
    .translate([0, 0, STEM_DEPTH / 2 - 0.25]);
  const crossB = Manifold.cube([CROSS_W, CROSS_L, STEM_DEPTH + 0.5], true)
    .translate([0, 0, STEM_DEPTH / 2 - 0.25]);
...
const layout: KeySpec[] = [
  ...
  [0, 0, 1, 1],
  [2, 0, 1, 1], [3, 0, 1, 1], [4, 0, 1, 1], [5, 0, 1, 1],
  [6.5, 0, 1, 1], [7.5, 0, 1, 1], [8.5, 0, 1, 1], [9.5, 0, 1, 1],
  [11, 0, 1, 1], [12, 0, 1, 1], [13, 0, 1, 1], [14, 0, 1, 1],
  [15.25, 0, 1, 1], [16.25, 0, 1, 1], [17.25, 0, 1, 1],
  [18.5, 0, 1, 1], [19.5, 0, 1, 1], [20.5, 0, 1, 1], [21.5, 0, 1, 1],
  ...
  [10, 1.25, 1, 1], [11, 1.25, 1, 1], [12, 1.25, 1, 1],
  [13, 1.25, 2, 1], // Backspace
  [15.25, 1.25, 1, 1], [16.25, 1.25, 1, 1], [17.25, 1.25, 1, 1],
  [18.5, 1.25, 1, 1], [19.5, 1.25, 1, 1], [20.5, 1.25, 1, 1], [21.5, 1.25, 1, 1],
  // Row 2 — QWERTY
  [0, 2.25, 1.5, 1], // Tab
  [1.5, 2.25, 1, 1], [2.5, 2.25, 1, 1], [3.5, 2.25, 1, 1], [4.5, 2.25, 1, 1], [5.5, 2.25, 1, 1],
  [6.5, 2.25, 1, 1], [7.5, 2.25, 1, 1], [8.5, 2.25, 1, 1], [9.5, 2.25, 1, 1], [10.5, 2.25, 1, 1],
  [11.5, 2.25, 1, 1], [12.5, 2.25, 1, 1],
  [13.5, 2.25, 1.5, 1], // backslash
  [15.25, 2.25, 1, 1], [16.25, 2.25, 1, 1], [17.25, 2.25, 1, 1],
  [18.5, 2.25, 1, 1], [19.5, 2.25, 1, 1], [20.5, 2.25, 1, 1],
  [21.5, 2.25, 1, 2], // Numpad +
  // Row 3 — ASDF
  [0, 3.25, 1.75, 1], // Caps
  [1.75, 3.25, 1, 1], [2.75, 3.25, 1, 1], [3.75, 3.25, 1, 1], [4.75, 3.25, 1, 1], [5.75, 3.25, 1, 1],
  [6.75, 3.25, 1, 1], [7.75, 3.25, 1, 1], [8.75, 3.25, 1, 1], [9.75, 3.25, 1, 1], [10.75, 3.25, 1, 1],
  [11.75, 3.25, 1, 1],
  [12.75, 3.25, 2.25, 1], // Enter
  [18.5, 3.25, 1, 1], [19.5, 3.25, 1, 1], [20.5, 3.25, 1, 1],
  // Row 4 — ZXCV
  [0, 4.25, 2.25, 1], // LShift
  [2.25, 4.25, 1, 1], [3.25, 4.25, 1, 1], [4.25, 4.25, 1, 1], [5.25, 4.25, 1, 1], [6.25, 4.25, 1, 1],
  [7.25, 4.25, 1, 1], [8.25, 4.25, 1, 1], [9.25, 4.25, 1, 1], [10.25, 4.25, 1, 1], [11.25, 4.25, 1, 1],
  [12.25, 4.25, 2.75, 1], // RShift
  [16.25, 4.25, 1, 1], // Up arrow
  [18.5, 4.25, 1, 1], [19.5, 4.25, 1, 1], [20.5, 4.25, 1, 1],
  [21.5, 4.25, 1, 2], // Numpad Enter
  // Row 5 — bottom row
  [0, 5.25, 1.25, 1], [1.25, 5.25, 1.25, 1], [2.5, 5.25, 1.25, 1],
  [3.75, 5.25, 6.25, 1], // Space
  [10, 5.25, 1.25, 1], [11.25, 5.25, 1.25, 1], [12.5, 5.25, 1.25, 1], [13.75, 5.25, 1.25, 1],
  [15.25, 5.25, 1, 1], [16.25, 5.25, 1, 1], [17.25, 5.25, 1, 1],
